  ...
  this->convertTac(tac);
        ...
        void Gas::setFunctionStackSpace(Label::ptr_t functionLabel,
          ...
          if (result != functionStackSpaceMap->end()) {
            ...
            functionStackSpaceMap->insert(
                std::make_pair(functionLabel, stackSpace));
          }
        }
            ...
            if (helper::isType<Variable>(op)) {
              ...
              if (triple->containsArg2()) {
                ...
                if (helper::isType<IntLiteral>(op2)) {
                  ...
                  auto asmVar2 =
                      std::make_shared<Operand>(Register::EBP, varOffset2);

                  asmInstructions.push_back(std::make_shared<Mnemonic>(
                      Instruction::MOV, eax, asmVar2));

                  asmInstructions.push_back(std::make_shared<Mnemonic>(
                      Instruction::MOV, asmVar, eax));
                }
              }
            }
          }
        ...
        void Gas::convertDiv(Triple::ptr_t triple) {
          ...
          asmInstructions.push_back(
              std::make_shared<Mnemonic>(Instruction::CDQ));
          ...
          asmInstructions.push_back(
              std::make_shared<Mnemonic>(Instruction::IDIV, ecx));
