...
TEST(Libmc, Literals) {
    ...
    EXPECT_MATCH(parser::int_literal, str_1, [](auto res) {
        return res->value == 1;
    });
    EXPECT_MATCH(parser::float_literal, str_4dot5, [](auto res) {
        return res->value == 4.5;
    });
}
...
TEST(Libmc, BinOperands) {
    ...
    EXPECT_MATCH(parser::binary_operand, str_plus, [](auto res) {
        return *res == ast::binary_operand::ADD; });
    EXPECT_MATCH(parser::binary_operand, str_mul, [](auto res) {
        return *res == ast::binary_operand::MUL; });
}

TEST(Libmc, Expressions) {
    EXPECT_NO_MATCH(parser::literal, str_test);
    EXPECT_MATCH_TYPED(parser::literal, str_1, ast::int_literal, [](auto res) {
        return res->value == 1; });
    EXPECT_MATCH_TYPED(parser::literal, str_4dot5, ast::float_literal, [](auto res) {
        return res->value == 4.5; });
    ...
    EXPECT_MATCH(parser::binary_operation, str_addition, [](auto res) {
        return *res->op == ast::binary_operand::ADD && *res->lhs == ast::int_literal {
            1
        }; });
    ...
    EXPECT_MATCH(parser::unary_operation, str_neg, [](auto res) {
        return *res->op == ast::unary_operand::MINUS && *res->sub == ast::float_literal {
            4.2
        }; });
...
TEST(Libmc, statements) {
    ...
    EXPECT_MATCH(parser::expr_stmt, str_expr_stmt, [](auto res) {
        return *res->sub == ast::int_literal {
            1
        }; });

    EXPECT_NO_MATCH(parser::compound_stmt, str_test);
    EXPECT_MATCH(parser::compound_stmt, str_empty_compound, [](auto res) {
        return res->statements.empty(); });
    EXPECT_MATCH(parser::compound_stmt, str_compound, [](auto res) {
        return res->statements.size() == 2; });

    EXPECT_NO_MATCH(parser::if_stmt, str_test);
    EXPECT_MATCH(parser::if_stmt, str_if, [](auto res) {
        return *res->condition == ast::int_literal {
            1
        }; });
    EXPECT_MATCH(parser::if_stmt, str_if_else, [](auto res) {
        return *res->else_stmt == ast::expr_stmt {
            std::make_shared<ast::int_literal>(5)
        }; });
    EXPECT_MATCH(parser::decl_stmt, str_decl, [](auto res) {
        return res->var->name == "test" && typeid (*res->var->var_type) == typeid (ast::int_type); });
