...
namespace cfg {
    ...
    Scg::Scg(mcc::tac::Tac &tac) {
        ...
        for(auto triple : tac.codeLines) {
            if(typeid(*triple) == typeid(Label)) {
                ...
                auto targetFunction = std::static_pointer_cast<Label>(triple->getArg1());
                boost::add_edge(graphIndex[currentFunction->getName()], graphIndex[targetFunction->getName()], graph);
            }
        }
    ...
    std::string Scg::toDot() const {
        ...
        auto name = boost::make_transform_value_property_map(Name{}, boost::get(boost::vertex_bundle,graph));
