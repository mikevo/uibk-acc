...
namespace mcc {
    namespace lvn {
        ...
        void LVN::addTempVarAssignment(Variable::ptr_t keyVar, tempAssignValue keyValue, Tac &tac,
                unsigned offset) {

            auto triple = std::make_shared<Triple>(Operator(OperatorName::ASSIGN),
                    keyVar, keyValue.first);
        ...
        Variable::ptr_t LVN::addTempVar(unsigned position, Variable::ptr_t var) {
            ...
            LVN::tempVarAssign.insert(std::make_pair(tempVar, std::make_pair(var, position)));
